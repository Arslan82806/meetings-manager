...
class Skypemeeting extends StatefulWidget {
  final String user;

  const Skypemeeting({Key key, this.user}) : super(key: key);
  @override
  _SkypemeetingState createState() => _SkypemeetingState();
}

class _SkypemeetingState extends State<Skypemeeting> {
  String facebook;
  bool meeting = false;
  ...
  getdata() async {
    ...
    await adddata.doc(widget.user).get().then((documentSnapshot) {
      print(widget.user);
      facebook = documentSnapshot.data()['skype'];
      print(facebook);
    }
